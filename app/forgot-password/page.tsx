...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const message = error instanceof AxiosError ? error.response?.data?.message : "Unable to send reset instructions"
      setErrorMessage(message || "Unable to send reset instructions")
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-sky-50 to-blue-100 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center p-4">
      <div className="w-full max-w-md rounded-2xl border border-gray-100 bg-white p-8 shadow-2xl dark:border-gray-700 dark:bg-gray-800">
        ...
        <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">Enter your email. We will send a reset link if account exists.</p>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-5">
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input name="email" type="email" required autoComplete="email" className="h-12 pl-12" placeholder="you@example.com" />
            </div>
          </div>
        ...
        <p className="mt-6 text-center text-sm text-gray-600 dark:text-gray-400">
          <Link href="/login" className="font-medium text-sky-600 hover:text-sky-700 dark:text-sky-400">Back to sign in</Link>
        </p>
      </div>
    </div>
  )
