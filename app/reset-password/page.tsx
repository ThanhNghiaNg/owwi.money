...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const message = error instanceof AxiosError ? error.response?.data?.message : "Unable to reset password"
      setErrorMessage(message || "Unable to reset password")
    }
  }
      ...
      <div className="w-full max-w-md rounded-2xl border border-gray-100 bg-white p-8 shadow-2xl dark:border-gray-700 dark:bg-gray-800">
        ...
        {!token ? (
          <div className="mt-6 space-y-4">
            <p className="text-sm text-red-600 dark:text-red-400">Reset token is missing.</p>
            <Link href="/forgot-password" className="font-medium text-sky-600 hover:text-sky-700 dark:text-sky-400">Request new link</Link>
          </div>
        ) : (
