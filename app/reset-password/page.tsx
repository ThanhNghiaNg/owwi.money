...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const message = error instanceof AxiosError ? error.response?.data?.message : t("auth.unableToResetPassword")
      setErrorMessage(message || t("auth.unableToResetPassword"))
    }
  }
        ...
        {!token ? (
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"><KeyRound /></span>
                <Input name="confirmPassword" type="password" required minLength={6} autoComplete="new-password" className="h-12 pl-12" />
              </div>
            </div>
